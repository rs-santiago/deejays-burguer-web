<template>
  <Teleport to="body">
    <transition name="fade">
      <div v-if="cart.isHistoryOpen" class="fixed inset-0 bg-black/95 z-[100] flex items-end sm:items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-neutral-900 w-full max-w-md rounded-[2.5rem] p-8 border border-white/10 shadow-2xl relative">
          
          <button @click="cart.isHistoryOpen = false" class="absolute top-6 right-6 text-neutral-500 hover:text-white transition-colors">✕</button>
          ...
          <div v-else>
            ...
            <div v-if="cart.orderHistory.length > 0" class="max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar space-y-4 text-left">
              <div v-for="order in cart.orderHistory" :key="order.id" class="bg-black/40 border border-white/5 rounded-[2rem] p-6">
                <div class="flex justify-between items-start mb-4">
                  ...
                  <span class="bg-amber-500 text-black text-[9px] font-black px-2.5 py-1 rounded-lg uppercase tracking-tighter">
                    {{ order.status }}
                  </span>
                </div>
                ...
                <div class="border-t border-white/5 pt-4 mt-4 space-y-2">
                  ...
                  <button @click="cart.repeatOrder(order.items)" class="w-full mt-6 py-4 rounded-2xl border border-amber-500/20 text-amber-500 text-[9px] font-black uppercase tracking-[0.2em] hover:bg-amber-500 hover:text-black transition-all">
                    Repetir este pedido ↺
                  </button>
                </div>
              </div>
            ...
            <div class="mt-8 pt-6 border-t border-white/5 flex flex-col items-center gap-3">
              ...
              <button @click="handleLogout" class="text-red-500/40 hover:text-red-500 text-[9px] font-black uppercase tracking-widest transition-colors">
