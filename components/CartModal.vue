<template>
  <Teleport to="body">
    <transition name="fade">
      <div v-if="cart.isModalOpen" class="fixed inset-0 bg-black/95 z-[100] flex items-end sm:items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-neutral-900 w-full max-w-md rounded-[2.5rem] p-8 border border-white/10 shadow-2xl relative">
          
          <button @click="cart.isModalOpen = false" class="absolute top-6 right-6 text-neutral-500 hover:text-white transition-colors">✕</button>
          ...
          <div class="max-h-[40vh] overflow-y-auto mb-8 pr-2 custom-scrollbar">
            <div v-for="item in cart.currentCartItems" :key="item.id" class="flex justify-between items-center py-4 border-b border-white/5">
              <div class="flex flex-col">
                <span class="text-white font-bold text-sm">{{ item.quantity }}x {{ item.name }}</span>
                <span class="text-neutral-500 text-[10px] uppercase">R$ {{ (item.price * item.quantity).toFixed(2) }}</span>
              </div>
              <button @click="cart.removeFromCart(item.id)" class="text-red-500/50 text-[10px] font-black uppercase px-2">Remover</button>
            </div>
          </div>
          ...
          <div class="mb-8">
            <label class="text-neutral-500 text-[9px] uppercase tracking-[0.3em] font-black mb-3 block">Confirme seu WhatsApp</label>
            <input 
...
const sendOrder = () => {
  if (!cart.isPhoneValid) return alert("WhatsApp inválido")

  let msg = `*PEDIDO - ${props.brand.name}*%0A`
  msg += `*Cliente:* ${cart.customerPhone}%0A`
  msg += `--------------------------%0A`
