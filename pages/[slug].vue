...
const handleAuthAction = () => {
  if (cart.customerPhone) {
    ...
    // Abre o modal de histórico (que agora terá o campo de login se estiver vazio)
    cart.isHistoryOpen = true
  }
}
...
<template>
  <div v-if="brand" class="min-h-screen text-white font-sans scroll-smooth"
      ...
      <div class="flex justify-between items-center p-6 max-w-7xl mx-auto">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <button @click="showScheduleModal = true" class="flex items-center gap-2 mt-1 group">
              <span :class="isStoreOpen ? 'text-green-500' : 'text-red-500'"
                class="text-[9px] font-black uppercase tracking-widest flex items-center gap-1">
              ...
              <span
                class="text-neutral-500 text-[9px] font-bold uppercase underline decoration-white/10 group-hover:text-white transition-colors">Ver
                horários</span>
            </button>
          </div>
        </div>
        <div class="space-x-8 hidden md:flex font-medium text-sm uppercase tracking-widest">
          ...
          <button @click="handleAuthAction"
            class="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full hover:bg-white/10 transition-all">
      ...
      <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center z-10 py-20">
        ...
        <div class="relative group justify-self-center">
          <NuxtImg :src="brand.hero.image"
            class="rounded-2xl transform group-hover:rotate-1 transition-transform duration-700 w-full max-lg shadow-2xl"
            preload />
        </div>
      </div>
    ...
    <section id="menu" class="max-w-7xl mx-auto px-6 py-24 border-t border-white/5">
      <div class="text-center mb-12">
        <h2 class="text-4xl md:text-6xl font-black uppercase mb-4">Nosso <span
            :style="{ color: brand.colors.primary }">Cardápio</span></h2>
        <p class="text-neutral-500 uppercase tracking-[0.2em] text-xs font-bold text-center">As faixas mais pedidas da
          casa</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <ProductCard v-for="item in filteredProducts" :key="item.id" :product="item" :brand="brand"
          :themeColor="brand.colors.primary" :disabled="!isStoreOpen" />
      </div>
    </section>
    ...
    <section id="contact" class="max-w-7xl mx-auto px-6 py-24 border-t border-white/5">
      <div class="bg-neutral-900/30 rounded-[3rem] p-8 md:p-16 border border-white/5 relative overflow-hidden">
        <div class="relative z-10 grid md:grid-cols-2 gap-12">
          <div>
            <h2 class="text-4xl font-black uppercase mb-6">Manda um <span
                :style="{ color: brand.colors.primary }">Salve</span></h2>
            <div class="space-y-6">
              <a :href="`https://wa.me/${brand.contact.whatsapp}`" target="_blank"
                class="flex items-center gap-4 group">
                <div
                  class="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center transition-all group-hover:bg-white/10">
                  <span class="text-xs font-bold" :style="{ color: brand.colors.primary }">WA</span>
                </div>
                <div><span
                    class="block text-xs uppercase text-neutral-500 font-bold tracking-widest">WhatsApp</span><span
                    class="text-white text-lg font-medium">{{ brand.contact.whatsappDisplay }}</span></div>
              </a>
              <a :href="brand.contact.instaLink" target="_blank" class="flex items-center gap-4 group">
                ...
                <div><span
                    class="block text-xs uppercase text-neutral-500 font-bold tracking-widest">Instagram</span><span
                    class="text-white text-lg font-medium">{{ brand.contact.instagram }}</span></div>
              </a>
            </div>
          </div>
          <div
    ...
    <Transition name="fade">
      <div v-if="showScheduleModal"
        ...
        <div class="bg-neutral-900 border border-white/10 w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative">
          ...
          <h3 class="text-xl font-black uppercase italic mb-8">Horários de <span
              :style="{ color: brand.colors.primary }">Funcionamento</span></h3>
          <div class="space-y-1">
          ...
          <button @click="showScheduleModal = false"
            class="w-full mt-8 py-5 rounded-2xl font-black uppercase text-xs tracking-[0.3em]"
