...
if (!brand.value || error.value || !brand.value.isActive) {
  ...
  // 🔥 NOVO: Injeção de Meta Tags (SEO e Open Graph para WhatsApp/Redes Sociais)
  useSeoMeta({
    title: `${brand.value.name} ${brand.value.surname || ''} | Menu`,
    ogTitle: `${brand.value.name} ${brand.value.surname || ''}`,
    description: brand.value.hero?.description || brand.value.tagline || 'Confira nosso cardápio e faça seu pedido online!',
    ogDescription: brand.value.hero?.description || brand.value.tagline || 'Confira nosso cardápio e faça seu pedido online!',
    ogImage: brand.value.hero?.image || brand.value.logoUrl, // Pega a capa, ou a logo se a capa não existir
    twitterCard: 'summary_large_image',
    themeColor: brand.value.colors?.primary || '#F59E0B'
  })
}
...
<template>
  <div v-if="brand" class="min-h-screen text-white font-sans scroll-smooth flex flex-col overflow-x-hidden"
    ...
    <ProductModal :isOpen="isProductModalOpen" :product="selectedProduct" @close="isProductModalOpen = false"
      @add-to-cart="handleAddToCart" />

    <div v-if="!isStoreOpen"
      class="bg-red-600 text-center py-2 text-[10px] font-black uppercase tracking-[0.2em] sticky top-0 z-[70] shadow-2xl">
    ...
    <nav class="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/5">
      <div class="flex justify-between items-center p-6 max-w-7xl mx-auto w-full">
        <div class="flex items-center gap-4">
          <div v-if="brand.logoUrl"
            class="w-12 h-12 rounded-xl overflow-hidden border border-white/10 bg-neutral-900 shadow-inner">
          ...
          <div>
            ...
            <button @click="showScheduleModal = true"
              class="text-neutral-500 text-[9px] font-bold uppercase underline mt-1">Ver horários</button>
          </div>
        </div>
        ...
        <div class="hidden md:flex items-center space-x-8 font-bold text-xs uppercase tracking-widest">
          <button @click="setTab('menu')"
            :class="activeTab === 'menu' ? 'text-white' : 'text-neutral-500'">Cardápio</button>
          <button @click="setTab('info')" :class="activeTab === 'info' ? 'text-white' : 'text-neutral-500'">Sobre &
            Contato</button>
          <button @click="handleAuthAction"
            class="text-amber-500 border border-white/10 px-4 py-2 rounded-full bg-white/5">Pedidos</button>
        </div>
      </div>
      ...
      <Transition name="fade">
        <div v-if="isMobileMenuOpen" class="md:hidden bg-neutral-950 p-8 space-y-6 flex flex-col items-center">
          <button @click="setTab('menu')" class="text-lg font-black uppercase italic"
            :class="activeTab === 'menu' ? 'text-white' : 'text-neutral-500'">Cardápio</button>
          <button @click="setTab('info')" class="text-lg font-black uppercase italic"
            :class="activeTab === 'info' ? 'text-white' : 'text-neutral-500'">Sobre & Contato</button>
          <button @click="handleAuthAction(); isMobileMenuOpen = false"
            class="w-full bg-white/5 border border-white/10 py-4 rounded-full text-amber-500 font-black text-xs">Meus
            Pedidos</button>
        </div>
      </Transition>
    </nav>
    ...
    <main class="flex-1 w-full overflow-x-hidden">
      ...
      <div v-if="activeTab === 'menu'">
        <header class="relative min-h-[60vh] flex items-center overflow-hidden">
          ...
          <div class="absolute top-0 right-0 w-1/2 h-full opacity-10 blur-[120px] -z-0"
        ...
        <div v-if="pending" class="flex justify-center py-20">
          <div class="w-10 h-10 border-4 border-t-transparent rounded-full animate-spin"
            :style="{ borderColor: brand.colors.primary }"></div>
        </div>
        ...
        <div v-else class="max-w-7xl mx-auto px-6 pb-24 space-y-24 w-full">
          <section v-for="category in menuByCategories" :key="category.id" :id="category.slug" class="scroll-mt-32">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              ...
              <ProductCard v-for="product in category.products" :key="product.id" :product="product" :brand="brand"
                :themeColor="brand.colors.primary" :disabled="!isStoreOpen" @click="openProductModal(product)"
      ...
      <div v-if="activeTab === 'info'" class="w-full">
        ...
        <section class="max-w-7xl mx-auto px-6 py-24">
          <div class="grid md:grid-cols-2 gap-16 items-center">
            <div class="relative">
              <NuxtImg :src="brand.hero.image"
                class="rounded-[3rem] grayscale hover:grayscale-0 transition-all duration-700 shadow-2xl w-full" />
              <div class="absolute -bottom-6 -right-6 p-8 rounded-2xl hidden md:block text-black font-black"
            ...
            <div class="space-y-6">
              <h2 class="text-4xl md:text-6xl font-black uppercase italic leading-none">
                {{ brand.about.title }} <span :style="{ color: brand.colors.primary }">{{ brand.about.highlight
                  }}</span>
              </h2>
              <p class="text-neutral-400 leading-relaxed text-lg">{{ brand.about.description }}</p>
              <div class="grid grid-cols-2 gap-4 pt-4">
                <div v-for="feat in brand.about.features" :key="feat"
                  class="border border-white/10 p-4 rounded-xl text-center">
        ...
        <section class="max-w-7xl mx-auto px-6 py-24 border-t border-white/5 w-full">
          <div class="bg-neutral-900/30 rounded-[3rem] p-8 md:p-16 border border-white/5 relative overflow-hidden">
            <div class="relative z-10 grid md:grid-cols-2 gap-12 text-center md:text-left">
              <div>
                <h2 class="text-4xl font-black uppercase mb-6 text-white">Manda um <span
                    :style="{ color: brand.colors.primary }">Salve</span></h2>
                <div class="space-y-6 flex flex-col items-center md:items-start">
                  <a :href="`https://wa.me/${brand.contact.whatsapp}`" target="_blank"
                    class="flex items-center gap-4 group">
                    <div class="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center font-bold"
                      :style="{ color: brand.colors.primary }">WA</div>
                    <div>
                  ...
                  <a :href="brand.contact.instaLink" target="_blank" class="flex items-center gap-4 group">
                    <div class="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center font-bold"
                      :style="{ color: brand.colors.primary }">IG</div>
                    <div>
              ...
              <div
                class="bg-neutral-950 rounded-[2rem] p-8 border border-white/5 flex flex-col items-center justify-center text-center">
                <span class="text-neutral-400 font-bold text-xs uppercase tracking-[0.3em] leading-relaxed italic">{{
                  brand.contact.location }}</span>
              </div>
            </div>
          </div>
        </section>
    ...
    <footer class="bg-neutral-900/50 border-t border-white/5 py-12 mt-auto">
      <div class="max-w-7xl mx-auto px-6 text-center">
        <p class="text-white font-black uppercase tracking-widest text-[10px]">Desenvolvido por <span
            :style="{ color: brand.colors.primary }">Rodrigo Santiago</span></p>
      </div>
    </footer>
        ...
        <div class="bg-neutral-900 border border-white/10 w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative">
          ...
          <div class="space-y-1">
            <div v-for="s in brand.schedules.sort((a, b) => a.dayOfWeek - b.dayOfWeek)" :key="s.dayOfWeek"
              ...
              <span class="font-bold uppercase text-[11px] tracking-[0.2em]"
                :class="new Date().getDay() === s.dayOfWeek ? 'text-white' : 'text-neutral-500'">{{
                  getDayName(s.dayOfWeek) }}</span>
              <div v-if="s.closed"
                class="text-[9px] font-black bg-red-500/10 text-red-500 px-3 py-1 rounded-lg uppercase">Fechado</div>
              <div v-else class="text-xs font-mono font-bold text-neutral-300">{{ s.openTime }} — {{ s.closeTime }}
              </div>
            </div>
          </div>
          <button @click="showScheduleModal = false" class="w-full mt-8 py-5 rounded-2xl font-black uppercase text-xs"
            :style="{ backgroundColor: brand.colors.primary, color: '#000' }">Fechar</button>
        </div>
      </div>
    </Transition>
